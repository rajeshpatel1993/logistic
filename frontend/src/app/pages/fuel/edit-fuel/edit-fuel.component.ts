...
@Component({
  selector: 'ngx-edit-fuel',
  templateUrl: './edit-fuel.component.html',
  styleUrls: ['./edit-fuel.component.scss']
})
export class EditFuelComponent implements OnInit {
  ...
  constructor(private router: Router, private dialogService: NbDialogService,private fb: FormBuilder, private vehicleService : VehicleService, public vehicleservService : VehicleservService, private fuelService: FuelService, private activeRoute: ActivatedRoute) { }

  ngOnInit() {
    this.fuelId = this.activeRoute.snapshot.params.id;
    this.createForm(this.fuelId);
    this.loadVehiclesTypes();
    this.loadEmployee();
    this.loadPaymentMethods();
    this.loadFuel(this.fuelId);
  ...
  updateFuel(){
    ...
    this.fuelService.updateFuelEntry(this.fuelEntryForm.value).subscribe((data)=>{
      // console.log(data);
  ...
  loadFuel(fuelId){
    ...
    this.fuelService.loadFuelEntry(fuelId).subscribe((d)=>{
      let fuelData = d['data'][0];
      this.fuelEntryForm.get("vehiclename").patchValue(fuelData.vehicleData[0].name);
      this.selectEventVehicle(fuelData.vehicle);
      this.fuelEntryForm.get("expiration_date").patchValue(fuelData.expiration_date);
      this.fuelEntryForm.get("expiration_time").patchValue(fuelData.expiration_time);
      this.fuelEntryForm.get("modeofpayment").patchValue(fuelData.paymentModeData[0]._id);
      this.fuelEntryForm.get("amount").patchValue(fuelData.amount);
      this.fuelEntryForm.get("odometer").patchValue(fuelData.odometer);
      this.fuelEntryForm.get("type").patchValue(fuelData.type);
      this.fuelEntryForm.get("priceunit").patchValue(fuelData.priceunit);
      this.fuelEntryForm.get("unit").patchValue(fuelData.unit);
      this.fuelEntryForm.get("vendorname").patchValue(fuelData.vendorname);
      this.fuelEntryForm.get("drivername").patchValue(fuelData.employeeData[0].firstName);
      this.fuelEntryForm.get("comment").patchValue(fuelData.comment);
      this.fuelEntryForm.get("bill_file_unique_id").patchValue(fuelData.bill_file_unique_id);
      this.fuelEntryForm.get("image_file_unique_id").patchValue(fuelData.image_file_unique_id);
